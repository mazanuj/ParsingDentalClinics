        ...
        public static async Task<IEnumerable<InfoHolder>> GetInfo()
        {
            try
            {
                ...
                var pageLast =
                    int.Parse(
                        doc.DocumentNode.Descendants("div")
                            .First(
                                x => x.Attributes.Contains("class") && x.Attributes["class"].Value == "b-paging--inner")
                            .ChildNodes.Last(
                                x => x.Attributes.Contains("class") && x.Attributes["class"].Value == "b-paging__link")
                            .InnerText);
                ...
                for (var i = pageLast; i > 0; i--)
                {
                    doc =
                        new HtmlWeb().Load(
                            string.Format("http://www.kz.all.biz/stomatologicheskie-uslugi-bsr1675?page={0}", i));
                    var clinics =
                        doc.DocumentNode.Descendants("div")
                            .Where(
                                x =>
                        ...
                        var infoHolder = new InfoHolder
                                             {
                                                 ...
                                                 ClinicName =
                                                     ...
                                                     .First(
                                                         ...
                                                         && x.Attributes["itemprop"].Value == "name")
                                                     .InnerText),
                                                 Phone =
                                                     RegExHelper.RegExpression(
                                                         string.Join(
                                                 ...
                                                 Address =
                                                     ...
                                                     .First(
                                                         ...
                                                         && x.Attributes["class"].Value == "country-name")
                                                     .InnerText,
                                                             contacts.Descendants("span")
                                                     .First(
                                                         x =>
                                                         x.Attributes.Contains("class")
                                                         && x.Attributes["class"].Value == "locality")
                                                     .InnerText,
                                                             contacts.Descendants("span")
                                                     .First(
                                                         x =>
            ...
            {
                Console.WriteLine("Error in AllBizKzData {0}", ex.Message);
