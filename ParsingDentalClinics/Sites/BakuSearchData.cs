    ...
    internal class BakuSearchData
    {
        public static async Task<IEnumerable<InfoHolder>> GetInfo()
        {
            try
            {
                ...
                var htmlByte =
                    await
                    new WebClient().DownloadDataTaskAsync("http://www.bakusearch.info/russian/medical/stom_clinic.php");
                ...
                foreach (var table in tables)
                {
                    ...
                    try
                    {
                        td =
                            table.Descendants("td")
                                .First(
                                    x =>
                                    x.Attributes.Contains("class") && x.Attributes["class"].Value == "black_verdana_11")
                                .InnerText.Split('\n')
                    ...
                    var infoHolder = new InfoHolder
                                         {
                                             ...
                                             ClinicName =
                                                 table.Descendants("div")
                                                 .First(
                                                     x =>
                                                     x.Attributes.Contains("class")
                                                     && x.Attributes["class"].Value == "blue_verdana_12")
                                                 .InnerText
                                         };
                    foreach (var value in td)
                    {
                        ...
                        {
                            if (!string.IsNullOrEmpty(infoHolder.Phone)) infoHolder.Phone += "; " + RegExHelper.RegExpression(value);
                            else infoHolder.Phone = RegExHelper.RegExpression(value);
                        }
                        else if (value.Contains("e-mail")) infoHolder.Mail = RegExHelper.RegExpression(value.Replace("e-mail:", ""));
                        else if (!value.Contains("URL")) infoHolder.Address = RegExHelper.RegExpression(value);
                    }
            ...
            {
                Console.WriteLine("Error in BakuSearchData {0}", ex.Message);
