        ...
        public static async Task<IEnumerable<InfoHolder>> GetInfo()
        {
            return await Task.Run(
                () =>
                {
                                    ...
                                    case 0:
                                        holder.ClinicName =
                                            RegExHelper.RegExpression(
                                                tds[j].ChildNodes.Where(y => y.InnerText != string.Empty)
                                                    .Aggregate(
                                                        string.Empty,
                                                        (current, htmlNode) => current + htmlNode.InnerText));
                                        break;
                                    case 1:
                                        var text =
                                            RegExHelper.RegExpression(
                    ...
                    {
                        Console.WriteLine("Error in InterstomData {0}", ex.Message);
